...
tifFilePath = fullfile(folderPath, baseFileName);
...
coordsCsvFilePath = fullfile(coordsFolderPath, [name, '.csv']);

% Paths to CSV files for cells and brain landmarks
for ch = 1:length(channels) 
    
csvFilePath = fullfile(csvFolderPath, [channels{ch},'_',name, '.csv']); %this will be channel dependent
...
if isfile(csvFilePath) && isfile(coordsCsvFilePath)
    ...
    cells.channel = repelem(channels(ch),size(cells, 1)).';
    ...
    warning(['CSV files for channel ',channels{ch},' for file ', name, ' are missing.']);
end
